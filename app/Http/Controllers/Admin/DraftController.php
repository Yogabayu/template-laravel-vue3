...
class DraftController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            return ResponseHelper::successRes('Berchapai Update data', $draft);
        } catch (\Exception $e) {
    ...
    public function addDraftComment(Request $request)
    {
        try {
            ...
            foreach ($fcm_tokens as $token) {
                try {
                    $messaging = app('firebase.messaging');
                    $notification = Notification::create('Komentar ditambahkan', $draft->title . ' silahkan di lakukan pengecekan');
                    $message = CloudMessage::withTarget('token', $token)
                        ->withNotification($notification);
                    $messaging->send($message);
                } catch (MessagingException $ex) {
