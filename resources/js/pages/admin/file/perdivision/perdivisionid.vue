    ...
    <VCard class="auth-card pa-4 pt-5">
      <VCardItem class="align-left">
        ...
        <VCardTitle class="text-2xl font-weight-bold">
          Daftar File Berdasarkan Divisi {{ divisionName ?? "-" }}
        </VCardTitle>
      </VCardItem>
...
export default {
  ...
  methods: {
    ...
    async getAllFilePerDivisionId(id: any) {
      try {
        const response = await mainURL.get(`/fileperdivisionid/${id}`);
        if (response.status === 200) {
          this.items = response.data.data.files;
          this.divisionName = response.data.data.name;
        } else {
  ...
  mounted() {
    this.getAllFilePerDivisionId(this.divisionId);
  },
};
</script>
