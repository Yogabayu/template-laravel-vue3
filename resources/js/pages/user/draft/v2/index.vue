<template>
  <v-card>
    ...
    <v-card-text>
          ...
          <EasyDataTable class="mt-2" show-index :headers="headers" :items="items" :search-value="searchValue"
            ...
            <template #item-operation="item">
              <div class="operation-wrapper">
                ...
                &nbsp;
                <button v-if="userData.uuid == item.user_uuid">
              ...
              <v-col class="ma-2" v-for="(draft, index) in items" :key="index" cols="12" sm="3">
                <v-card class="mx-auto" max-width="344" height="230px">
                  ...
                  <v-card-actions>
                    ...
                    <v-btn size="small" class="mt-3" @click="openModal(2, draft)" v-if="userData.uuid == draft.user_uuid">
      ...
      { value: 'approved', title: 'Approved' },
      { value: 'rejected', title: 'Rejected' },
    ]" v-model="dataForm.status" prepend-icon="mdi-help-rhombus">
                      </v-select>
                    </VCol>
      ...
      if (!this.isTable) {
        if (newVal !== "") {
          this.items = this.originalItems.filter(
            (item) => item.title.includes(newVal)
          );
        } else {
  ...
  methods: {
    ...
    async insertData() {
      try {
        ...
        const response = await mainURL.post("/user/draft", formData, config);
    ...
    async deleteData(item) {
      try {
        ...
        const response = await mainURL.delete(`/user/draft/${item.id}`);
    ...
    async getAllDraft() {
      try {
        const response = await mainURL.get("/user/draft");
