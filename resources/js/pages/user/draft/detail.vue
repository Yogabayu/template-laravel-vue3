<template>
  ...
  <VCard class="auth-card pa-2 pt-5"><span color="primary" @click="goBack" style="cursor: pointer">
      <VIcon icon="bx-arrow-back" color="primary" tag="back" start />
      Back
    </span>
    <VCardTitle class="text-2xl font-weight-bold d-flex justify-center overflow-auto">
      {{ draft.title }}
    </VCardTitle>
    <v-alert v-if="draft.status == 'approved'" text="Draft Telah dinyatakan disetujui oleh semua pihak"
      title="Draft Telah disetujui" type="success"></v-alert>
  </VCard>
  ...
  <VCard class="auth-card pt-5">
    <v-container fluid>
      <v-row no-gutters>
        ...
        <v-col cols="12" sm="2" class="pa-0">
          <v-chip class="ma-2" color="success" prepend-icon="mdi-checkbox-marked-circle"
            v-if="draft.status == 'approved'" @click="changeDraftStatus(draft.id)">
            Approved
          </v-chip>
          <v-chip class="ma-2" color="warning" prepend-icon="mdi-alert" v-if="draft.status == 'pending'"
            @click="changeDraftStatus(draft.id)">
            Pending
          </v-chip>
          <v-chip class="ma-2" color="error" prepend-icon="mdi-alert" v-if="draft.status == 'rejected'">
            Rejected
          </v-chip>
        </v-col>
      </v-row>
      ...
      <v-row>
        <v-col cols="12" sm="10" class="px-2 py-2">
          <a :href="draftUrl + '/' + draft?.file" target="_blank">
            <v-btn :prepend-icon="getFileExtension(draft?.file) === 'pdf'
    ? 'mdi-file-pdf'
    : 'mdi-file-word'
    " size="small">
              {{ draft?.file }}
  ...
  <VCard class="auth-card pt-5">
    <v-container fluid>
      ...
      <v-row no-gutters>
        <v-col cols="12" sm="12" class="pa-0">
          <v-chip v-for="(pos, index) in selectedPositions" :color="pos?.pivot?.is_approved ? 'success' : 'error'"
            :key="index" class="ma-2" @click="openModal(1, pos)">{{ pos.name }}</v-chip>
        </v-col>
      </v-row>
    </v-container>
  </VCard>
  ...
  <VCard class="auth-card pt-5">
    <v-container fluid>
            ...
            <v-card-title>
              <VForm @submit.prevent="addComment">
                <v-row align="center" justify="center">
                  <v-col md="10" cols="10">
                    <v-textarea bg-color="grey-lighten-2" color="cyan" v-model="dataComment.desc"></v-textarea>
                  </v-col>
                  <v-col md="2" cols="2" class="d-flex justify-space-between align-center">
                    <v-btn icon color="primary" @click="addComment">
                      <v-icon>mdi-send</v-icon>
                    </v-btn>
                    <v-btn icon color="secondary" size="small" @click="attachment = !attachment">
                      <v-icon>mdi-paperclip</v-icon>
                    </v-btn>
                  </v-col>
                  <v-col cols="12" v-if="attachment">
                    <v-file-input accept="application/pdf,
                      application/msword,
                      application/vnd.openxmlformats-officedocument.wordprocessingml.document,
                      application/vnd.ms-excel,
                      application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" prepend-icon="mdi-file"
                      label="File Attachment" @change="handleFileChange" ref="fileInput"></v-file-input>
                  </v-col>
                </v-row>
              </VForm>
            </v-card-title>
      ...
      <v-row>
        <v-col>
          <v-container>
            ...
            <v-row>
              <v-col cols="12" sm="4">
                <v-chip color="primary" v-if="displayComment == false" @click="displayComment = !displayComment">Semua
                  Komentar</v-chip>
                <v-chip color="secondary" v-if="displayComment == true" @click="displayComment = !displayComment">Semua
                  Komentar</v-chip>
              </v-col>
              <v-col cols="12" sm="4">
                <v-chip color="primary" v-if="displayComment == true"
                  @click="displayComment = !displayComment">Perposisi</v-chip>
                <v-chip color="secondary" v-if="displayComment == false"
                  @click="displayComment = !displayComment">Perposisi</v-chip>
              </v-col>
            </v-row>
          </v-container>
      ...
      <!-- <v-row>
        <v-col> -->
      <v-card variant="outlined" color="secondary" v-if="displayComment == true">
        <div v-if="difPos">
          <v-row>
            <v-col>
              <v-expansion-panels>
                <v-expansion-panel>
                  ...
                  <div v-for="(comment, index) in comments" :key="index">
                    <v-expansion-panel-text v-if="userData.position.id == comment.user.position_id">
                      <v-card class="mx-auto" color="secondary" variant="flat">
                        <v-card class="ma-4" rounded="lg" variant="flat">
                          ...
                          <div class="pa-4 d-flex align-center">
                            <div v-if="userData.uuid == comment.user.uuid">
                              <button>
                                <VIcon size="20" icon="bx-edit" color="blue" @click="openModal(2, comment)" />
                              </button>
                            </div>
                            <div v-if="userData.uuid == comment.user.uuid">
                              <button>
                                <VIcon size="20" icon="bx-trash" color="red" @click="deleteComment(comment.id)" />
                              </button>
                            </div>
                            <v-spacer></v-spacer>
                            <a :href="drafCommenttUrl + comment.attachment" target="_blank" v-if="comment.attachment">
                              <v-btn class="me-2 text-none" prepend-icon="mdi-paperclip" variant="flat"
              ...
              <v-expansion-panels>
                <v-expansion-panel>
                  <v-expansion-panel-title>
                    {{ pos.name }}
                    <v-chip class="ma-2" color="success" prepend-icon="mdi-checkbox-marked-circle"
                      v-if="pos.pivot.is_approved == 1" @click="openModal(1, pos)">
                      Approved
                    </v-chip>
                    <v-chip class="ma-2" color="error" prepend-icon="mdi-alert" @click="openModal(1, pos)" v-else>
                      Not Approved
                    </v-chip>
                  </v-expansion-panel-title>
                  <div v-for="(comment, index) in comments" :key="index">
                    <v-expansion-panel-text v-if="pos.id == comment.user.position_id">
                      <v-card class="mx-auto" color="secondary" variant="flat">
                        <v-card class="ma-4" rounded="lg" variant="flat">
                          <v-card-item>
                            <v-card-title class="text-body-2 d-flex align-center">
      ...
      <v-row>
        <v-col>
          <v-card variant="outlined" color="secondary" v-if="displayComment == false">
            <div>
              <v-row>
                <v-col>
                  <v-expansion-panels>
                            ...
                            <v-card class="mx-auto" color="secondary" variant="flat">
                              <v-card class="ma-4" rounded="lg" variant="flat">
                                ...
                                <div class="pa-4 d-flex align-center">
                                  ...
                                  <v-spacer></v-spacer>
                                  <a :href="drafCommenttUrl + comment.attachment" target="_blank"
                                    v-if="comment.attachment">
                                    <v-btn class="me-2 text-none" prepend-icon="mdi-paperclip" variant="flat">
                                      {{
    comment.attachment.substring(0, 10) + "..."
  }}
                                    </v-btn>
                                  </a>
                                </div>
